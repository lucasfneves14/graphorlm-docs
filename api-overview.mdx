...
Here’s the full "happy path": **upload → process → list → elements → chat**.
...
// Ingestion + Chat workflow
async function ingestAndChat() {
  try {
    ...
    const result = await client.ask("Summarize this document", [file.name]);
...
def ingest_and_chat(documents: List[str]):
    try:
        # 1. Ingest
        for doc_path in documents:
            ...
            # 2. Chat
            answer = api.ask("What is this document about?", [file_name])
            print(f"Answer for {file_name}: {answer['answer']}")
