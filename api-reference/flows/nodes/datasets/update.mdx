...
description: 'Modify dataset node configurations in flows via the Graphor REST API'
---

Update the configuration of a specific dataset node within a flow in your Graphor project. This endpoint allows you to modify which files are included in a dataset node and automatically marks the node for reprocessing.
