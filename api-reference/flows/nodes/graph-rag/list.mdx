...
description: 'Retrieve graph RAG nodes from flows via the Graphor REST API'
---

Retrieve all graph RAG nodes from a specific flow in your Graphor project. Graph RAG nodes are advanced RAG (Retrieval-Augmented Generation) components that combine document processing, chunking, NLP graph extraction, entity recognition, and intelligent retrieval operations into a single, knowledge graph-powered unit.
