...
description: 'Modify LLM node settings including model selection and prompt templates via the Graphor REST API'
---

Update the configuration of an LLM node in your Graphor flow. LLM nodes are sophisticated response generation components that transform retrieved context into natural language answers, serving as the final stage in RAG pipelines for delivering high-quality, contextually-aware responses.
