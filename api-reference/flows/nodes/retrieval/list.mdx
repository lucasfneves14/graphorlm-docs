...
description: 'Retrieve retrieval nodes from flows via the GraphorLM REST API'
---

Retrieve all retrieval nodes from a specific flow in your GraphorLM project. Retrieval nodes are critical components that perform similarity search and document retrieval operations, forming the core of RAG (Retrieval-Augmented Generation) systems by finding relevant content based on user queries.
...
<CardGroup cols={2}>
  <Card
    ...
    icon="sliders"
    href="/api-reference/flows/retrieval/update"
  >
