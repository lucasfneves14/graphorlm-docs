...
description: 'Complete guide to managing documents in Graphor via the REST API'
---
...
<CardGroup cols={2}>
  ...
  >
    **POST** `https://sources.graphorlm.com/upload`
    ...
    Upload documents from your local system to Graphor for processing
  </Card>
...
Graphor offers multiple processing methods, selectable via the [Process Source](/api-reference/sources/process) endpoint:
...
<AccordionGroup>
  ...
  <Accordion icon="brain" title="Hi-Res Method">
    **Speed**: ⚡ **Accuracy**: ⭐⭐⭐⭐
  ...
  <Accordion icon="sparkles" title="Hi-Res FT Method">
    **Speed**: ⚡ **Accuracy**: ⭐⭐⭐⭐⭐
  ...
  <Accordion icon="project-diagram" title="Graphor Method">
    **Speed**: ⚡ **Accuracy**: ⭐⭐⭐⭐
  ...
  <Accordion icon="bars" title="MAI Method">
    **Speed**: ⚡⚡⚡ **Accuracy**: ⭐⭐⭐⭐⭐
...
graph LR
    ...
    C -->|No| E[Reprocess with Hi-Res]
    E --> F[Review Again]
    F --> G{Better?}
    G -->|Yes| D
    G -->|No| H[Try Graphor Method]
```

1. Start with Basic method for speed
2. Review extraction quality
3. Upgrade to Hi-Res or Hi-Res FT if needed
...
| Complex Documents | Hi-Res or Hi-Res FT | Better structure recognition; consider Graphor for semantic relationships |
...
| Presentations | Hi-Res or Hi-Res FT | Better slide layout recognition |
...
<AccordionGroup>
  ...
  <Accordion icon="gears" title="Processing Strategy">
    **Method Selection**:
    ...
    - Reserve Hi-Res FT for premium accuracy needs
    - Prefer MAI when you don't need bounding boxes/layout metadata and want speed with top text parsing
    - Use Graphor for enhanced semantic relationships and graph-based organization
...
if status == 'completed':
    ...
    print(f"Reprocessed with Hi-Res: {process_result['partition_method']}")
```
...
class DocumentQualityPipeline {
  ...
  async assessAndOptimize(fileName) {
    const methods = ['basic', 'ocr', 'yolox', 'advanced', 'graphorlm', 'mai'];
    let bestResult = null;
    let bestScore = 0;
...
<AccordionGroup>
  <Accordion icon="question-circle" title="Getting Help">
    ...
    - **Email Support**: [support@graphorlm.com](mailto:support@graphorlm.com)
