...
<CardGroup cols={2}>
  ...
  >
    **POST** `https://sources.graphorlm.com/upload`
    ...
    Upload sources to Graphor (files, web pages via URL, and GitHub repositories)
  </Card>
  
  <Card
    title="Upload Source from URL"
    icon="link"
    href="/api-reference/sources/upload#upload-from-url"
  >
  ...
  <Card
    ...
    icon="code-branch"
    href="/api-reference/sources/upload#upload-from-github"
  >
...
<AccordionGroup>
  ...
  <Accordion icon="brain" title="Balanced Method">
    **Speed**: ⚡ **Accuracy**: ⭐⭐⭐⭐
  ...
  <Accordion icon="sparkles" title="Accurate Method">
    **Speed**: ⚡ **Accuracy**: ⭐⭐⭐⭐⭐
  ...
  <Accordion icon="project-diagram" title="Agentic Method">
    **Speed**: ⚡ **Accuracy**: ⭐⭐⭐⭐⭐
  ...
  <Accordion icon="bars" title="VLM Method">
    **Speed**: ⚡⚡⚡ **Accuracy**: ⭐⭐⭐⭐⭐
...
```mermaid
graph LR
    A[Upload with Fast] --> B[Review Results]
    B --> C{Quality Good?}
    C -->|Yes| D[Use Document]
    C -->|No| E[Reprocess with Balanced]
    E --> F[Review Again]
    F --> G{Better?}
    G -->|Yes| D
    G -->|No| H[Try Agentic Method]
```
