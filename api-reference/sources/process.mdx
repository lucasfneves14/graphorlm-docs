---
title: 'Parse Source'
description: 'Reprocess uploaded documents with different parsing methods via the Graphor REST API'
---
...
|-------|------|-------------|----------|
| `file_id` | string | Unique identifier for the source (preferred) | No* |
| `file_name` | string | Name of the previously uploaded file to reprocess (deprecated, use `file_id`) | No* |
...
| `file_id` | string | Unique identifier for the source |
...
<AccordionGroup>
  ...
  <Accordion icon="exclamation-triangle" title="Invalid Method (400)">
    ...
    **Solution**: Use one of: basic, hi_res, hi_res_ft, graphorlm, mai
  </Accordion>
</AccordionGroup>
