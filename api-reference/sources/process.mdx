...
| `method` | string | Partitioning strategy. One of: `auto`, `fast`, `balanced`, `accurate`, `agentic`. Default: `fast` | No |
| `enrichment` | string | Annotation level: `full` (default) or `none` (see [Enrichment and indexing options](/api-reference/sources/upload#enrichment-and-indexing-options)) | No |
| `indexing` | string | Index level: `full` (default) or `none`. With `none`, the source is parsed but not searchable (see [Enrichment and indexing options](/api-reference/sources/upload#enrichment-and-indexing-options)) | No |

<Warning>
  Re-processing with `indexing: 'none'` drops the source's current chunks — the source stops being searchable until you re-process it with indexing enabled or index the build via **POST** `https://sources.graphorlm.com/index` (see [Index build](/api-reference/sources/index-build)).
</Warning>
...
|-------|------|-------------|
| `auto` | Auto | Per-page routing: classifies each page and runs the cheapest parser that fits (Fast for plain text, Accurate for tables, Agentic for scans). Best balance of cost and quality on mixed PDFs. PDF-only. |
...
<AccordionGroup>
  <Accordion icon="wand-magic-sparkles" title="Auto">
...
|--------|-------|--------------|-------------------------|----------------|-----------------|-----|
| **Auto** | Mixed (per page) | Best for the page | Best for the page | Yes (when needed) | Mixed PDFs with body + tables + scans | When needed |
