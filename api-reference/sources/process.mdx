...
<AccordionGroup>
  ...
  <Accordion icon="brain" title="Hi-Res">
  ...
  <Accordion icon="sparkles" title="Hi-Res FT">
...
<Warning>
Processing can take several minutes depending on document size, complexity, and the selected processing method. Advanced methods like Hi-Res, Hi-Res FT, GraphorLM and MAI typically require more time for analysis.
</Warning>
...
<AccordionGroup>
  ...
  <Accordion icon="exclamation-triangle" title="Invalid Method (400)">
    ...
    **Solution**: Use one of: basic, ocr, yolox, advanced, graphorlm, mai
  </Accordion>
</AccordionGroup>
...
- **Upgrade gradually**: Move to OCR → Hi-Res → Hi-Res FT -> GraphorLM -> MAI based on needs
...
def improve_processing_quality(api_token, file_name):
    ...
    methods = ['basic', 'ocr', 'yolox', 'advanced', 'graphorlm', 'mai']
