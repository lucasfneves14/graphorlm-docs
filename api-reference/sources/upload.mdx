...
When the build is still in progress or the ID is invalid (no history yet):
...
| `status` | string | `Completed`, `Processing`, `Processing failed`, or `not_found` when no history exists |
