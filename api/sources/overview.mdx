...
### 1. Upload Stage
When you upload a document using the [Upload Source](/api/sources/upload) endpoint:
...
GraphorLM offers multiple processing methods, selectable via the [Process Source](/api/sources/process) endpoint:
...
Documents progress through various states that you can monitor using the [List Sources](/api/sources/list) endpoint:
...
1. **Upload**: Use [Upload Source](/api/sources/upload) to add your document
2. **Monitor**: Check status with [List Sources](/api/sources/list)
3. **Optimize**: Reprocess with [Process Source](/api/sources/process) if needed
