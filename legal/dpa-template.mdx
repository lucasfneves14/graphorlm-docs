...
Synapse ensures that all personnel authorized to Process Customer Personal Data are bound by appropriate confidentiality obligations (LGPD art. 47 / GDPR art. 28(3)(b)).
...
- **Google Cloud Platform** — the [Google Cloud DPA](https://cloud.google.com/terms/data-processing-addendum) for all GCP-hosted components.
- **Neo4j AuraDB** — the [Neo4j DPA](https://neo4j.com/legal/data-protection-addendum/) for the managed knowledge graph.
...
Synapse implements and maintains the technical and organizational measures described in [Annex 2](#annex-2--technical-and-organizational-measures), which include encryption at rest (Google-managed AES-256; CMEK on enterprise request), encryption in transit (TLS 1.2+), logical tenant isolation, per-project API tokens with TTL and audit, and the operational controls inventoried in [Compliance §3](/trust/compliance#3-compensating-controls-inventory).
...
The 72-hour SLA aligns with LGPD art. 48 ("prazo razoável," interpreted by ANPD as 72 hours) and with GDPR art. 33 (controller-to-supervisory-authority window).
...
|--------------|---------|---------------|
| Encryption at rest | Google-managed AES-256 by default; CMEK available on enterprise request | [Trust Center Overview §3](/trust/overview#3-encryption-posture) |
| Encryption in transit | TLS 1.2+ enforced on every public surface; Google-managed certificates | [Architecture §5](/trust/architecture#5-public-network-surface) |
| Logical tenant isolation | Single-tenant logical model with per-layer (API, Postgres, Neo4j, Storage, observability) project-scoped enforcement | [Tenant Isolation §1](/trust/tenant-isolation#1-isolation-by-layer) |
...
| Disaster recovery | Cloud SQL automated backups + PITR (7-day window); restore procedure that preserves customer DSR actions | [Data Retention §5](/trust/data-retention#5-backups-and-disaster-recovery) |
| Audit logging | Customer-visible activity log surface; export path | [Audit Logs](/trust/audit-logs) |
...
For the UK extension: the **International Data Transfer Addendum to the EU Commission Standard Contractual Clauses** (UK IDTA) is incorporated by reference and prevails in case of conflict with the SCCs for transfers covered by UK GDPR.

A copy of the full SCC text and the UK IDTA can be obtained on request from [privacy@graphorlm.com](mailto:privacy@graphorlm.com).
