...
| `get_build_status` | `build_id` | `suppress_elements`, `suppress_img_base64`, `page`, `page_size` | build status + `file_id` when complete |
...
<AccordionGroup>
  <Accordion icon="arrow-up-from-bracket" title="Ingest Documents">
    ...
    **Tool flow:**
    ...
    2. Call `get_build_status` with `build_id` repeatedly until `success: true`
  ...
  <Accordion icon="magnifying-glass" title="Semantic Search (RAG)">
    Use `retrieve_chunks` to find the most relevant text chunks for a query without generating an answer. Useful for building custom RAG pipelines or inspecting what context the model would use.
