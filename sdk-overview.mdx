---
title: 'SDK Overview'
description: 'Complete overview of the Graphor SDKs for Python and TypeScript: Sources, Chat, Extraction, and RAG'
---
...
<Tabs>
  ...
  <Tab title="TypeScript">
    ...
    |--------|-------------|
    | [`client.sources.upload()`](/sdk/sources/upload#upload-a-file) | Upload a local file |
    | [`client.sources.uploadURL()`](/sdk/sources/upload#upload-from-url) | Upload from a web URL |
    | [`client.sources.uploadGitHub()`](/sdk/sources/upload#upload-from-github) | Upload from GitHub |
    | [`client.sources.uploadYoutube()`](/sdk/sources/upload#upload-from-youtube) | Upload from YouTube |
    | [`client.sources.parse()`](/sdk/sources/process) | Reprocess a source with different parsing method |
    | [`client.sources.list()`](/sdk/sources/list) | List all sources in the project |
    | [`client.sources.delete()`](/sdk/sources/delete) | Delete a source permanently |
    | [`client.sources.loadElements()`](/sdk/sources/list-elements) | Get parsed elements from a source |
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="TypeScript">
    ```typescript
    // Reprocess with a different parsing method
    const parsed = await client.sources.parse({
      file_name: 'document.pdf',
      partition_method: 'hi_res',
    });
    console.log(`Processed with: ${parsed.partition_method}`);
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="TypeScript">
    ...
    // Find specific source
    const target = sources.find((s) => s.file_name === 'document.pdf');
    if (target) {
...
<Tabs>
  ...
  <Tab title="TypeScript">
    ...
    for (const item of elements.items) {
      console.log(`  [${item.metadata?.type}] ${item.page_content.slice(0, 100)}...`);
    }
    ```
  </Tab>
</Tabs>
...
<Tabs>
  ...
  <Tab title="TypeScript">
    ...
    console.log(`Invoice: ${result.structured_output?.invoice_number}`);
    console.log(`Amount: $${result.structured_output?.total_amount}`);
    ```
  </Tab>
</Tabs>
  ...
  <Tab title="TypeScript">
    ...
    class GraphorSDK {
      ...
      async uploadFile(filePath: string) {
        const source = await this.client.sources.upload({ file: fs.createReadStream(filePath) });
        return {
      ...
      async uploadUrl(url: string, crawl = false) {
        const source = await this.client.sources.uploadURL({ url, crawlUrls: crawl });
        return { fileName: source.file_name, status: source.status };
      }

      async process(fileName: string, method: string = 'hi_res') {
        const source = await this.client.sources.parse({
          file_name: fileName,
      ...
      async getElements(fileName: string, page = 1, pageSize = 50) {
        const result = await this.client.sources.loadElements({
          file_name: fileName,
          page,
          page_size: pageSize,
        });
      ...
      async delete(fileName: string) {
        ...
        return { fileName: result.file_name, message: result.message };
      }

      // ==================== Chat ====================

      async ask(question: string, fileNames?: string[], conversationId?: string) {
      ...
      async extract(fileNames: string[], instruction: string, schema: Record<string, unknown>) {
      ...
      async retrieve(query: string, fileNames?: string[]) {
    ...
    async function fullWorkflow(filePath: string) {
      try {
        ...
        // 2. Process
        const processResult = await sdk.process(fileName, 'hi_res');
        console.log(`Processed with: ${processResult.method}`);

        // 3. Chat
        const chatResult = await sdk.ask('Summarize this document', [fileName]);
...
<Tabs>
  ...
  <Tab title="TypeScript">
    ...
    class AsyncGraphorSDK {
      ...
      async processMultiple(filePaths: string[], method = 'hi_res') {
        const processOne = async (filePath: string) => {
          try {
            const source = await this.client.sources.upload({
              file: fs.createReadStream(filePath),
            });

            const processed = await this.client.sources.parse({
              file_name: source.file_name,
              partition_method: method as any,
            });

            return { file: filePath, status: 'success' as const, fileName: processed.file_name };
          } catch (err) {
        ...
        // Process all files concurrently
        return Promise.all(filePaths.map(processOne));
      }

      async batchAsk(questions: string[], fileNames?: string[]) {
...
<Tabs>
  ...
  <Tab title="TypeScript">
    ...
    try {
      const source = await client.sources.upload({ file: fs.createReadStream('file.pdf') });
    } catch (err) {
  ...
  <Tab title="TypeScript">
    ...
    async function robustUpload(filePath: string, maxAttempts = 3) {
      for (let attempt = 0; attempt < maxAttempts; attempt++) {
        try {
          const source = await client.sources.upload({ file: fs.createReadStream(filePath) });
          return { success: true, fileName: source.file_name };
        } catch (err) {
...
<Tabs>
    ...
    def document_pipeline(directory: str, method: str = "hi_res"):
        ...
        for file_path in Path(directory).glob("*.pdf"):
            try:
                ...
                print(f"Processed: {processed.file_name}")
  ...
  <Tab title="TypeScript">
    ...
    async function documentPipeline(directory: string, method = 'hi_res') {
      ...
      for (const file of files) {
        ...
        try {
          ...
          console.log(`Processed: ${processed.file_name}`);

          results.push({ file: filePath, status: 'success', fileName: processed.file_name });
        } catch (err) {
          const message = err instanceof Graphor.APIError ? err.message : String(err);
...
<Tabs>
  ...
  <Tab title="TypeScript">
    ...
    // Usage
    const qa = new DocumentQA(['report.pdf']);
...
<Tabs>
  ...
  <Tab title="TypeScript">
    ...
    async function customRag(question: string, fileNames?: string[]) {
      // 1. Retrieve relevant chunks
      const chunks = await graphorClient.sources.retrieveChunks({
        query: question,
        file_names: fileNames,
      });
      ...
      const context = (chunks.chunks ?? [])
        .map((c) => `[${c.file_name}, Page ${c.page_number}]\n${c.text}`)
        .join('\n\n');
    ...
    // Usage
    const result = await customRag('What are the payment terms?', ['contract.pdf']);
