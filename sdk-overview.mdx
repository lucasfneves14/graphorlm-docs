...
<Tabs>
  <Tab title="Python">
    ```python
    build_id = client.sources.reprocess(file_id=file_id, partition_method="balanced")
    print(f"Reprocessing: {build_id}")
    ```
  </Tab>
  <Tab title="TypeScript">
    ```typescript
    const reprocessBuildId = await client.sources.reprocess({ fileId, partitionMethod: 'balanced' });
...
<Tabs>
  ...
  <Tab title="TypeScript">
    ```typescript
    const elements = await client.sources.getElements({ fileId, page: 1, pageSize: 50 });
...
<Tabs>
  <Tab title="Python">
    ```python
    chunks = client.sources.retrieve_chunks(query="What are the payment terms?", file_ids=[file_id])
  ...
  <Tab title="Python">
    ...
    class GraphorSDK:
        ...
        def reprocess(self, file_id: str, method: str = "balanced") -> str:
            ...
            return self.client.sources.reprocess(file_id=file_id, partition_method=method)
  ...
  <Tab title="TypeScript">
    ...
    class GraphorSDK {
      ...
      async reprocess(fileId: string, method = 'balanced') {
        return this.client.sources.reprocess({ fileId, partitionMethod: method });
      }
      ...
      async getElements(fileId: string, page = 1, pageSize = 50) {
        const result = await this.client.sources.getElements({ fileId, page, pageSize });
        return {
          total: result.total,
          page: result.page,
...
<Tabs>
  ...
  <Tab title="TypeScript">
    ...
    await client.sources.reprocess(
      { fileId: 'file_abc123', partitionMethod: 'agentic' },
      { timeout: 300 * 1000 },
    );
    ```
  </Tab>
...
<Tabs>
  <Tab title="Python">
    ...
    def custom_search(question: str, file_ids: list[str] | None = None) -> dict:
        """Retrieve relevant chunks and use them as context."""
    ...
    result = custom_search("What are the payment terms?", ["file_abc123"])
    print(result["answer"])
  ...
  <Tab title="TypeScript">
    ...
    async function customSearch(question: string, fileIds?: string[]) {
      const chunks = await graphorClient.sources.retrieveChunks({
        query: question,
        fileIds,
      });
      const context = (chunks.chunks ?? [])
        .map((c) => `[${c.fileId}, Page ${c.page_number}]\n${c.text}`)
        .join('\n\n');
      ...
      return {
        answer: response.choices[0].message.content,
        sources: (chunks.chunks ?? []).map((c) => ({ fileId: c.file_id, page: c.page_number })),
      };
    }

    // Usage (pass fileIds from list())
    const result = await customSearch('What are the payment terms?', ['file_abc123']);
