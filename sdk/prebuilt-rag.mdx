...
client.sources.retrieve_chunks(
    query: str,                              # Required
    file_names: list[str] | None = None,     # Optional
...
| `file_names` | `list[str] \| None` | Restrict retrieval to specific documents by file name | No |
...
### Retrieval from Specific Documents
...
# Restrict retrieval to specific files
result = client.sources.retrieve_chunks(
...
1. **Use `file_names`** — Restrict to specific documents when you know the source
