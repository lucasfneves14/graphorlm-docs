...
The optional `enrichment` and `indexing` parameters control which stages of the pipeline run after parsing. Both accept `'full'` (default) or `'none'`.
...
<Warning>
  Reprocessing with `indexing: 'none'` drops the source's current chunks. The source stops being searchable until you reprocess it with indexing enabled or index it via the [Index build endpoint](/api-reference/sources/index-build).
</Warning>

`indexing: 'none'` requires deployment support: the GET sources config endpoint exposes `indexing_options`, and sending `'none'` on a deployment that does not support it returns a 400 error. The [Index build endpoint](/api-reference/sources/index-build) indexes a parse-only source later without re-parsing it.
...
### With ingestion options

Re-parse a source without LLM enrichment or indexing (see [Ingestion options](#ingestion-options)).
...
|------------|-------------|-------------|
| `BadRequestError` | 400 | Invalid request format or partition method, or `indexing: 'none'` on a deployment without indexing support |
