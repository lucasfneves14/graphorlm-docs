...
The response also reports how the build was processed:

- **`enrichment`** — The enrichment option the build ran with: `'full'` or `'none'` (see [Ingestion options](#ingestion-options)).
- **`indexing`** — The indexing option the build ran with: `'full'` or `'none'` (see [Ingestion options](#ingestion-options)).
...
<Note>
  The `enrichment`, `indexing`, and `searchable` fields are available on the REST API today and land in the next SDK release. Until then, read them from the raw JSON response if your installed SDK version does not expose them as typed fields.
</Note>
...
<Tabs>
  <Tab title="Python">
    ...
    client.sources.ingest_file(
        ...
        enrichment: str | None = None,               # Optional: full (default), none
        indexing: str | None = None,                 # Optional: full (default), none
        timeout: float | None = None
...
<Tabs>
  <Tab title="Python">
    ...
    | `enrichment` | `str \| None` | `'full'` (default) or `'none'` (see [Ingestion options](#ingestion-options) below) | No |
    | `indexing` | `str \| None` | `'full'` (default) or `'none'` (see [Ingestion options](#ingestion-options) below) | No |
  ...
  <Tab title="TypeScript">
    ...
    | `enrichment` | `string` | `'full'` (default) or `'none'` (see [Ingestion options](#ingestion-options) below) | No |
    | `indexing` | `string` | `'full'` (default) or `'none'` (see [Ingestion options](#ingestion-options) below) | No |
  </Tab>
</Tabs>
...
|-----------|--------|---------|-------------|
| `enrichment` | `'full'`, `'none'` | `'full'` | `'none'` skips LLM page, section, and document annotation. Parsing is faster and cheaper, but elements carry no LLM page annotations and retrieval/agent context is weaker. |
| `indexing` | `'full'`, `'none'` | `'full'` | `'none'` skips chunking, embedding, and indexing. The source is parsed but **not searchable**: ask, extract, and retrieve return nothing for it. Element reads still work. |
...
To make a parse-only source searchable later without re-parsing it, use the [Index build endpoint](/api-reference/sources/index-build).

<Note>
  `enrichment` and `indexing` are available on the REST API today and land in the next SDK release. If your installed SDK version does not accept them yet, pass them through the SDK's raw request options (`extra_body` in Python, request `body` in TypeScript).
</Note>
...
Skip LLM enrichment and indexing for a fast, parse-only ingestion (see [Ingestion options](#ingestion-options)).
...
<Note>
  These named parameters ship in the next SDK release; on current SDK versions, send them via the raw request options as described in [Ingestion options](#ingestion-options).
</Note>
...
<Tabs>
  <Tab title="Python">
    ...
    | `enrichment` | `str \| None` | `'full'` (default) or `'none'` (see [Ingestion options](#ingestion-options)) | No |
    | `indexing` | `str \| None` | `'full'` (default) or `'none'` (see [Ingestion options](#ingestion-options)) | No |
  ...
  <Tab title="TypeScript">
    ...
    | `enrichment` | `string` | `'full'` (default) or `'none'` (see [Ingestion options](#ingestion-options)) | No |
    | `indexing` | `string` | `'full'` (default) or `'none'` (see [Ingestion options](#ingestion-options)) | No |
  </Tab>
</Tabs>

<Note>
  `enrichment` and `indexing` are available on the REST API today and land in the next SDK release (see [Ingestion options](#ingestion-options)).
</Note>
