...
1. **Upload** a source — returns status `"New"`. If you set `partition_method` during upload, processing starts automatically.
2. **Process** the source by calling parse — synchronous, returns when done (`"New"` → `"Processed"`). Only required if `partition_method` was not set during upload.
3. **Query** the source (ask, extract, or retrieve chunks)

<Warning>
  If you did not set `partition_method` during upload, you must explicitly call parse to process the document. Parse is **synchronous** — when it returns, the document is ready to query.
</Warning>
...
| `parsing` | Process documents and check processing status |
...
1. Uses MCP tools for all operations
2. Uses `file_ids` over `file_names` (deprecated) — stores `file_id` from upload responses
3. Processes after upload if `partition_method` was not set during upload
...
- File upload patterns (`open()` file objects)
- Error handling (`graphor.BadRequestError`, `graphor.NotFoundError`, etc.)
...
<AccordionGroup>
  <Accordion icon="arrow-up-from-bracket" title="Upload and summarize a document">
    ...
    **Agent will**:
    ...
    2. Call parse to process the document (synchronous)
    3. Ask the document for a summary
  </Accordion>
  ...
  <Accordion icon="code" title="Write SDK code">
    ...
    **Agent will**:
    ...
    3. Include proper error handling and synchronous processing
  </Accordion>
