...
The Graphor production environment is organized in five layers. Customer interactions are authenticated at the edge, authorized at the application layer, and scoped to a single Project before any customer-content store is touched.
...
flowchart TB
    ...
    subgraph Data["Data layer"]
        ...
        DB["Relational and graph stores<br/>(metadata · vectors · history)"]
...
- **No customer-deployed cloud (BYOC).** See [Tenant Isolation §3](/trust/tenant-isolation#3-explicit-non-decisions) for the explicit non-decision.
