...
| **HIPAA** | Not in scope today | n/a | Graphor is not currently positioned as a HIPAA-Business-Associate. **Gap to BAA-eligibility** for a healthcare customer: (i) Google Cloud BAA (available — Synapse would execute), (ii) AWS BAA covering Bedrock (available — Synapse would execute), (iii) Stripe BAA (n/a — billing-only), (iv) re-scoping of self-hosted observability to a HIPAA-compatible posture (likely a tracing-off-only mode for PHI Projects), (v) customer-facing audit-log API (currently roadmap — needed to satisfy HIPAA §164.312(b)), (vi) breach-notification mapping to HIPAA §164.404 alongside the existing LGPD/GDPR notification. Not on roadmap today; revisit on first healthcare enterprise engagement via [privacy@graphorlm.com](mailto:privacy@graphorlm.com). |
...
| LGPD art. 39 (operator's duty to follow controller's instructions) + arts. 39–40 (operator's broader regime including subprocessor authorization) | Versioned subprocessor list with notification subscription; documented instructions captured in the Agreement, Trust Center, and DPA per [DPA §3.1](/legal/dpa-template) | [Subprocessors](/trust/subprocessors) |
| LGPD art. 48 (incident notification) | 72-hour contractual SLA meets or exceeds the **3-business-day** regulatory floor in ANPD Resolução CD/ANPD nº 15/2024, art. 5 | [Incident Response](/trust/incident-response) |
...
- **Documented backup-access procedure.** Production-access credentials are escrowed with a documented backup-access procedure for the event of founder incapacitation; the procedure names an alternate operations contact who can revoke credentials, freeze the production environment, and notify customers within the [Incident Response](/trust/incident-response) SLA. The named alternate contact is disclosed to enterprise customers under NDA.
...
- **No published RTO/RPO commitments today.** The 7-day backup window and provider DR posture provide an effective RPO ≤ 1 day for the relational database; an effective RTO for a full-region rebuild is on the order of hours and depends on the cloud-provider control plane. Quantified RTO/RPO targets will be published with a future revision of this page as runbook automation matures.
