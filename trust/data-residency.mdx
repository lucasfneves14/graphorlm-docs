...
- **Encryption at rest** on every backing store; cloud-provider-managed keys today, customer-managed encryption keys available on enterprise request (see [Tenant Isolation §1](/trust/tenant-isolation#1-isolation-by-layer) and [§7](/trust/tenant-isolation#7-personnel-security)). A request that compels the cloud provider rather than Synapse cannot decrypt customer data without the application-layer access path; customer-managed keys further reduce reachability.
...
### 4.3 Synapse's commitments under compelled disclosure

Per [DPA §9](/legal/dpa-template#9-cooperation-with-authorities), if Synapse receives a binding legal request from a competent authority for disclosure of Customer Personal Data:
...
Synapse will challenge overbroad or facially unlawful requests using available legal procedure (motion to quash, narrowing negotiations).

### 4.4 Brazilian Marco Civil interplay
...
- The Cláusulas-Padrão Contratuais incorporated in [DPA Annex 4-B](/legal/dpa-template#annex-4--international-transfer-instruments) bind Synapse contractually to LGPD-equivalent protections at the US processing destination.

### 4.5 Customer options for compounded risk reduction
...
- Request the **São Paulo region option** on the [Data Residency roadmap §5](#5-roadmap), which removes the US processing leg for the relational, object, and (where supported) graph stores.
