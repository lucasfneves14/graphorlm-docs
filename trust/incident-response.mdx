...
**Reporting channel.** [security@graphorlm.com](mailto:security@graphorlm.com) (also reachable via [privacy@graphorlm.com](mailto:privacy@graphorlm.com)). A `/.well-known/security.txt` file is published on the marketing site with the same channel and the current PGP key for encrypted reports.
...
**Acknowledgement.** Synapse acknowledges receipt within one business day, provides a status update within ten business days, and credits the reporter publicly (with their permission) once the issue is remediated. Bounty payments are not committed today but may be offered at Synapse's discretion for high-impact reports.
